...
def generate_content(topic: str) -> dict:
    ...
    try:
        ...
        title_response = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user",
                "content": f"Придумайте привлекательный и точный заголовок для статьи на тему '{topic}', с учётом актуальных новостей:\n{recent_news}. Заголовок должен быть интересным и ясно передавать суть темы. Не используй символ # в тексте, даже в заголовках."
            }],
            max_tokens=60,
            temperature=0.5,
            stop=["\n"]
        ...
        meta_response = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user",
                "content": f"Напишите мета-описание для статьи с заголовком: '{title}'. Оно должно быть полным, информативным и содержать основные ключевые слова. Не используй символ # в тексте, даже в заголовках."
            }],
            max_tokens=120,
            temperature=0.5,
            stop=["."]
